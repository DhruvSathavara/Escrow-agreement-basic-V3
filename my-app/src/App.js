...
function App() {
    ...
    const createAgreement = async () => {
        // Validate inputs
        if (agreementId == null || clientAddress == null || serviceProviderAddress == null || amount == null) {
        ...
        console.log(fund);
        const tx = await escroContract.createEscrowAgreement(agreementId, clientAddress, serviceProviderAddress, ethers.utils.parseEther(amount), { value :ethers.utils.parseEther(fund) });
    ...
    function ParsedAgreement(agreeId, clientAdd, providerAdd, agreementAmount, clientStake, completed, released, serviceProviderStake) {
    ...
    const fetchAgreementById = async (id) => {
        ...
        try {
            ...
            const agrmnt = new ParsedAgreement(id, agreement.client, agreement.serviceProvider, agreement.agreementAmount.toNumber(), agreement.clientStake.toNumber(), agreement.completed, agreement.fundsReleased, agreement.serviceProviderStake.toNumber())
